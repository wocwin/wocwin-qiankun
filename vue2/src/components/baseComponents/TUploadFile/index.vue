...
<template>
  <div class="upload-file-container">
    ...
    >
      ...
      <el-button v-else size="small" :disabled="btnDisabled" icon="el-icon-upload">{{ buttonName }}</el-button>
...
export default {
  ...
  props: {
    ...
    savePath: {
      type: String,
      default: `${process.env.VUE_APP_BASE_API}/mes/file/upload`
    },
    // 是否禁用
    disabled: {
      type: Boolean,
  ...
  methods: {
    ...
    beforeUpload (file) {
      ...
      // 当次上传文件数量不能大于10
      // if ((copyFileArr.length - fileArrLength) > this.onceLimit) {
      //   this.$messageUpload.error(`一次最多允许上传${this.onceLimit}个文件`)
      //   // this.fileArr = []
      //   return false
      // }
      // if (this.isOnceLimit) {
